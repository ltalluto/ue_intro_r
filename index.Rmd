...
<div class="left lt">
* Perhaps the the number of each species captured varies by year.
* We can see if the variable `year` is associated with the variable `species` using a **chi-squared test**.
...
boxplot(flipper_length_mm ~ island+sex, data = penguins_adelie, 
		at = c(1.6,2,2.4,    3.6,4,4.4), # at controls x-location, allows grouping by sex
		col = rep(cols, 2), # set the colours, repeated twice (female and male)
		cex.axis = 0.8, # axis label text size
		names = c("", "Female", "", "", "Male", ""), # labels under the boxes
		xlab = "", ylab = "Flipper length (mm)", # set axis titles
		bty = 'n', #disable the box around the plot
		notch = TRUE,
...
ggplot(penguins_adelie, aes(y = flipper_length_mm, x = sex, 
							fill = island)) + 
	geom_boxplot(notch = TRUE) + 
	theme_minimal() + ylab("Flipper length (mm)") + xlab("") +
...
ggplot(penguins_adelie, aes(y = flipper_length_mm, x = sex, fill = island)) + 
	geom_boxplot(notch = TRUE) + 
	theme_minimal() + ylab("Flipper length (mm)") + xlab("") +
	scale_fill_brewer(palette = "Paired") 
```
...
Show in ggplot how to divide data into margins

```{r fig.height=4, fig.width=4}
plot(flipper_length_mm ~ body_mass_g, data = penguins_adelie, pch=16, 
...
Useful for showing 3 variables

check ggplot cheatsheet (https://rstudio.github.io/cheatsheets/data-visualization.pdf) for more
